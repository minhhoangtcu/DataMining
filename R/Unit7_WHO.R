...
# Find the one region in particular has a lot of countries with a very low fertility rate and a very low percentage of the population under 15
ggplot(WHO, aes(x = FertilityRate, y = Under15, color = Region)) + geom_point(size = 3)
ggplot(WHO, aes(x = FertilityRate, y = Under15, color = Region)) + geom_point(size = 3) + scale_color_brewer(palette = "Dark2") # Easier to see as color blind person
